...
const Homepage = () => {
  ...
  const groupIsClicked = useSelector(
    (state) => state.userGroupCreation.groupIsClicked
  );

  const isToggle = useSelector((state) => state.userGroupCreation.isToggle);

  const chatScreenIsClicked = useSelector(
    (state) => state.userGroupCreation.chatScreenIsClicked
  );
      ...
      <Container className="inner-container">
        <Row>
          <Col md={4} lg={4} xs={12} className="user-list-col">
            ...
            <Stack direction="vertical" gap={5} className="m-4">
              <ListGroup className="user-list-group">
                <Stack direction="vertical" gap={1}>
                  <div
                    className="mx-auto mt-3 mb-4 border-bottom"
                    id="your-group-heading"
                  >
                    Specific Groups
                  </div>
                  {/* {groups.map((group) => (
              ...
              <ListGroup className="user-list-group">
                <Stack gap={1}>
                  <div
                    className="mx-auto mb-3 border-bottom"
                    id="your-group-heading"
                  >
                    All Users
                  </div>
                      ...
                      <Stack direction="horizontal">
                        <Stack direction="horizontal" gap={2}>
                          <Avatar
                            ...
                            className="avatar"
                          />
                          <div className="user-name me-auto">{user.name.toUpperCase()}</div>
                        </Stack>

                        <div className="user-lastseen ms-auto">
                          joined {formatLastActiveTime(user.updatedAt)}
                        </div>
                      </Stack>
                    </ListGroup.Item>
                  ))}
          ...
          <Col md={8} lg={8}>
            ...
            {isToggle && (
              <Stack direction="vertical" gap={2} className="mt-5">
                <div className="brand-slogan-homepage">
                  ChatCircle - Connect with Friends and Family Anytime,
                  Anywhere!
                </div>
                ...
                <div className="start-messaging-div">
                  <Button
                    ...
                    onClick={startMessagingHandler}
                  >
                    <Stack direction="horizontal" gap={2}>
                      <div className="start-messaging-txt">Start Messaging</div>
                      <div className="start-messaging-icon">
                        <FaPaperPlane />
                      </div>
                    </Stack>
                  </Button>
